...
import { Download, Star, HardDrive, Gamepad2, Info, Camera, Image as ImageIcon, RefreshCw, FileEdit } from 'lucide-react';
...
export const LaunchBoxCard: React.FC<LaunchBoxCardProps> = ({ game, onSelect, onOpenDownload }) => {
  ...
  const handleEditDescClick = (e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <>
      <motion.div
        ...
        exit={{ opacity: 0 }}
        className={`group glass-card rounded-2xl overflow-visible hover:-translate-y-1 transition-all duration-300 flex flex-col sm:flex-row h-auto sm:h-52 ${
      ...
      >
        ...
        <div
          ...
          className="relative w-full sm:w-44 h-52 sm:h-full shrink-0 bg-slate-900 overflow-hidden rounded-t-2xl sm:rounded-tr-none sm:rounded-l-2xl cursor-pointer flex items-center justify-center"
        >
          {isLoading ? (
          ...
          {isAdmin && (
            <button
              ...
              title="Upload / Đổi ảnh bìa lên Vercel Blob"
        ...
        {/* Right Content */}
        <div className="p-4 flex-1 flex flex-col justify-between bg-white/[0.04] backdrop-blur-md rounded-b-2xl sm:rounded-bl-none sm:rounded-r-2xl border-t sm:border-t-0 sm:border-l border-white/10 group-hover:bg-amber-500/10 transition-all duration-300">
          <div>
                ...
                {isAdmin && (
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="px-2 py-0.5 rounded-lg bg-cyan-500/20 hover:bg-cyan-500 hover:text-slate-950 border border-cyan-500/40 text-cyan-300 text-[10px] font-mono flex items-center gap-1 transition-all cursor-pointer"
                    >
                      <Camera className="w-3 h-3 text-cyan-400" />
                      <span className="hidden xs:inline">Đổi ảnh</span>
                    </button>
                    ...
                    <button
                      ...
                      className="px-2 py-0.5 rounded-lg bg-amber-500/20 hover:bg-amber-500 hover:text-slate-950 border border-amber-500/40 text-amber-300 text-[10px] font-mono flex items-center gap-1 transition-all cursor-pointer"
                    >
                      <FileEdit className="w-3 h-3 text-amber-400" />
                      <span className="hidden xs:inline">Sửa mô tả</span>
                    </button>
                  </div>
                )}
              </div>
            ...
            <p className="text-xs text-slate-400 line-clamp-2 mt-1 leading-relaxed">
              {currentDescription || `Ngôn ngữ: ${game.language}. Phiên bản Việt hóa Quán Game Xóm.`}
            </p>
